...
describe('Center Prompt Box', () => {
    it('accepts the mulligan', () => {
        ...
        const { webSocket } = render(<CenterPromptBox />, { preloadedState });
    ...
    it('displays who is actively mulliganing for the non-active players', () => {
        ...
        expect(screen.queryByText('Accept Mulligan')).not.toBeInTheDocument();
        expect(
            screen.queryByText('Tommy is deciding whether to keep their hand')
        ).toBeInTheDocument();
    });
});
